...
const CATEGORY_QUESTIONS: Record<GrievanceCategory, string[]> = {
  ...
  [GrievanceCategory.OTHER]: ['Please describe your issue in detail.'],
};
...
export class ChatbotService {
  ...
  async startGrievance(
    ...
    if (booking.status !== BookingStatus.COMPLETED) {
      throw new BadRequestException(
        'You can only raise grievances for completed bookings',
      );
    }
  ...
  async processMessage(
    ...
    if (categoryKeys.includes(customerMessage as GrievanceCategory)) {
      return this.handleCategorySelection(
        grievance,
        customerMessage as GrievanceCategory,
      );
    }
    ...
    return {
      message:
        'Your grievance is being reviewed by our team. An admin will contact you shortly.',
    };
  }
  ...
  private async handleInProgressState(
    ...
    const messageCount = grievance.messages.filter(
      (m) => m.sender === MessageSender.CUSTOMER,
    ).length;
    ...
    switch (category) {
      ...
      case GrievanceCategory.DAMAGED_PROPERTY:
        return this.escalateToAdmin(
          grievance,
          'Requires refund review by admin',
        );

      case GrievanceCategory.RUDE_BEHAVIOR:
        return this.escalateToAdmin(
          grievance,
          'Behavioral complaint requires admin review',
        );
  ...
  private async offerReschedule(grievance: Grievance): Promise<BotResponse> {
    const response: BotResponse = {
      message:
        'I understand you experienced a delay. Would you like me to reschedule your service?',
  ...
  private async offerRescheduleOrEscalate(
    grievance: Grievance,
  ): Promise<BotResponse> {
    const response: BotResponse = {
      message:
        "I understand the service wasn't up to your expectations. I can offer you:",
      options: [
  ...
  private async offerCouponOrEscalate(
    grievance: Grievance,
  ): Promise<BotResponse> {
    const response: BotResponse = {
      message: 'I apologize for the inconvenience. I can offer:',
      options: ['Get a 15% discount coupon', 'Speak to an admin'],
    };
  ...
  private async escalateToAdmin(
    ...
    const message =
      `I'm escalating your case to our admin team. ${reason}. ` +
  ...
  async handleOptionSelection(
    ...
    if (option.includes('admin') || option.includes('refund')) {
      return this.escalateToAdmin(
        grievance,
        'Customer requested admin assistance',
      );
    }
  ...
  private async processCoupon(grievance: Grievance): Promise<BotResponse> {
    ...
    const message =
      `Great news! I've generated a 15% discount coupon for your next booking.\n\n` +
