...
describe('PlatformFeesService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PlatformFeesService,
        { provide: getRepositoryToken(PlatformFeeConfig), useValue: feeConfigRepo },
        { provide: getRepositoryToken(ProviderSubscriptionPlan), useValue: planRepo },
        { provide: getRepositoryToken(ProviderSubscription), useValue: subscriptionRepo },
        { provide: getRepositoryToken(PromoCode), useValue: promoCodeRepo },
        { provide: getRepositoryToken(PromoCodeUsage), useValue: promoCodeUsageRepo },
        { provide: getRepositoryToken(CustomerMembershipPlan), useValue: membershipPlanRepo },
        { provide: getRepositoryToken(CustomerMembership), useValue: customerMembershipRepo },
  ...
  describe('Fee Config', () => {
    ...
    it('updateConfig replaces configValue entirely', async () => {
      const existing = { configKey: 'test', configValue: { type: 'fixed' }, isActive: true };
      feeConfigRepo.findOne.mockResolvedValue(existing);
      feeConfigRepo.save.mockImplementation((e: any) => Promise.resolve(e));

      const result = await service.updateConfig('test', { value: 50 }, undefined, 'admin-1');
    ...
    it('updateConfig throws NotFound when config missing', async () => {
      feeConfigRepo.findOne.mockResolvedValue(null);
      await expect(service.updateConfig('missing', {})).rejects.toThrow(NotFoundException);
    });

    it('isFeatureEnabled returns true only when active and enabled', () => {
      expect(service.isFeatureEnabled({ configKey: 'x', configValue: { enabled: true }, isActive: true } as any)).toBe(true);
      expect(service.isFeatureEnabled({ configKey: 'x', configValue: { enabled: false }, isActive: true } as any)).toBe(false);
      expect(service.isFeatureEnabled({ configKey: 'x', configValue: { enabled: true }, isActive: false } as any)).toBe(false);
  ...
  describe('Convenience Fee', () => {
    it('returns zero when feature is disabled', async () => {
      feeConfigRepo.findOne.mockResolvedValue({ configKey: 'feature_convenience_fee', configValue: { enabled: false }, isActive: true });
    ...
    it('calculates percentage fee', async () => {
      feeConfigRepo.findOne
        .mockResolvedValueOnce({ configKey: 'feature_convenience_fee', configValue: { enabled: true }, isActive: true })
        .mockResolvedValueOnce({ configKey: 'convenience_fee', isActive: true, configValue: { type: 'percentage', value: 5, minAmount: 10, maxAmount: 200 } });
      const result = await service.getConvenienceFee(1000);
    ...
    it('applies min/max caps', async () => {
      feeConfigRepo.findOne
        .mockResolvedValueOnce({ configKey: 'feature_convenience_fee', configValue: { enabled: true }, isActive: true })
        .mockResolvedValueOnce({ configKey: 'convenience_fee', isActive: true, configValue: { type: 'fixed', value: 5, minAmount: 10, maxAmount: 30 } });
    ...
    it('applies fixed fee', async () => {
      feeConfigRepo.findOne
        .mockResolvedValueOnce({ configKey: 'feature_convenience_fee', configValue: { enabled: true }, isActive: true })
        .mockResolvedValueOnce({ configKey: 'convenience_fee', isActive: true, configValue: { type: 'fixed', value: 25, minAmount: 0, maxAmount: 0 } });
  ...
  describe('Subscription Plans', () => {
    const dto = { name: 'Pro', monthlyPrice: 499, benefits: { commissionDiscount: 20 } };

    it('creates a plan', async () => {
      planRepo.create.mockReturnValue(dto as any);
      planRepo.save.mockResolvedValue(dto as any);
      const result = await service.createSubscriptionPlan(dto as any);
    ...
    it('updates a plan', async () => {
      ...
      const result = await service.updateSubscriptionPlan('p1', { name: 'Premium' });
      expect(result.name).toBe('Premium');
    });

    it('throws on update when not found', async () => {
      planRepo.findOne.mockResolvedValue(null);
      await expect(service.updateSubscriptionPlan('x', { name: 'X' })).rejects.toThrow(NotFoundException);
    });
  ...
  describe('Provider Subscriptions', () => {
    it('assigns a subscription to a provider', async () => {
      ...
      subscriptionRepo.create.mockReturnValue({ id: 'sub-1', status: 'active' } as any);
      subscriptionRepo.save.mockResolvedValue({ id: 'sub-1', status: 'active' } as any);

      const result = await service.assignSubscription('prov-1', 'plan-1', 'admin-1');
      expect(result.status).toBe('active');
      expect(activityLogsService.log).toHaveBeenCalled();
    });

    it('gets provider subscription', async () => {
      subscriptionRepo.findOne.mockResolvedValue({ id: 'sub-1', plan: { name: 'Pro' } });
      const result = await service.getProviderSubscription('prov-1');
      expect(result).toEqual({ id: 'sub-1', plan: { name: 'Pro' } });
    });

    it('cancels a subscription', async () => {
      subscriptionRepo.findOne.mockResolvedValue({ id: 'sub-1', plan: { name: 'Pro' }, status: 'active' });
  ...
  describe('Promo Codes', () => {
    ...
    it('creates a promo code', async () => {
      ...
      const result = await service.createPromoCode(dto as any);
      expect(result.code).toBe('SAVE20');
    });

    it('throws on duplicate code', async () => {
      promoCodeRepo.findOne.mockResolvedValue({ code: 'SAVE20' });
      await expect(service.createPromoCode(dto as any)).rejects.toThrow(BadRequestException);
    });
    ...
    describe('validatePromoCode', () => {
      ...
      it('returns valid for active promo', async () => {
        feeConfigRepo.findOne.mockResolvedValue({ configKey: 'feature_promo_codes', configValue: { enabled: true }, isActive: true });
      ...
      it('returns invalid when promo disabled', async () => {
        feeConfigRepo.findOne.mockResolvedValue({ configKey: 'feature_promo_codes', configValue: { enabled: false }, isActive: true });
        const result = await service.validatePromoCode('SAVE10', 'user-1', 500);
      ...
      it('returns invalid when code not found', async () => {
        feeConfigRepo.findOne.mockResolvedValue({ configKey: 'feature_promo_codes', configValue: { enabled: true }, isActive: true });
        promoCodeRepo.findOne.mockResolvedValue(null);
        const result = await service.validatePromoCode('INVALID', 'user-1', 500);
        expect(result.valid).toBe(false);
      ...
      it('returns fee_waiver discount as MAX_SAFE_INTEGER', async () => {
        feeConfigRepo.findOne.mockResolvedValue({ configKey: 'feature_promo_codes', configValue: { enabled: true }, isActive: true });
        promoCodeRepo.findOne.mockResolvedValue({ ...activePromo, type: 'fee_waiver', value: 0 });
      ...
      it('applies maxDiscount restriction', async () => {
        feeConfigRepo.findOne.mockResolvedValue({ configKey: 'feature_promo_codes', configValue: { enabled: true }, isActive: true });
        promoCodeRepo.findOne.mockResolvedValue({ ...activePromo, restrictions: { maxDiscount: 20 } });
        const result = await service.validatePromoCode('SAVE10', 'user-1', 1000);
        expect(result.discount).toBe(20);
      });

      it('rejects when min booking amount not met', async () => {
        feeConfigRepo.findOne.mockResolvedValue({ configKey: 'feature_promo_codes', configValue: { enabled: true }, isActive: true });
        promoCodeRepo.findOne.mockResolvedValue({ ...activePromo, restrictions: { minBookingAmount: 1000 } });
        const result = await service.validatePromoCode('SAVE10', 'user-1', 500);
        expect(result.valid).toBe(false);
      });
    });
    ...
    describe('getPromoCodeUsageHistory', () => {
      it('returns usage history', async () => {
        promoCodeUsageRepo.find.mockResolvedValue([{ id: 'usage-1', discountAmount: 50 }]);
  ...
  describe('Instant Payout Fee', () => {
    it('returns zero when feature disabled', async () => {
      feeConfigRepo.findOne.mockResolvedValue({ configKey: 'feature_instant_payout', configValue: { enabled: false }, isActive: true });
    ...
    it('calculates percentage fee', async () => {
      feeConfigRepo.findOne
        .mockResolvedValueOnce({ configKey: 'feature_instant_payout', configValue: { enabled: true }, isActive: true })
        .mockResolvedValueOnce({ configKey: 'instant_payout_fee', isActive: true, configValue: { type: 'percentage', value: 2.5, minAmount: 5, maxAmount: 500 } });
      const result = await service.calculateInstantPayoutFee(1000);
      expect(result.baseFee).toBe(25);
    ...
    it('applies min fee cap', async () => {
      feeConfigRepo.findOne
        .mockResolvedValueOnce({ configKey: 'feature_instant_payout', configValue: { enabled: true }, isActive: true })
        .mockResolvedValueOnce({ configKey: 'instant_payout_fee', isActive: true, configValue: { type: 'fixed', value: 2, minAmount: 10, maxAmount: 100 } });
  ...
  describe('Lead/Quote Fee', () => {
    it('returns zero when feature disabled', async () => {
      feeConfigRepo.findOne.mockResolvedValue({ configKey: 'feature_lead_quote_fee', configValue: { enabled: false }, isActive: true });
    ...
    it('returns zero when provider has subscription with free leads', async () => {
      feeConfigRepo.findOne
        .mockResolvedValueOnce({ configKey: 'feature_lead_quote_fee', configValue: { enabled: true }, isActive: true })
        .mockResolvedValueOnce({ configKey: 'lead_quote_fee', isActive: true, configValue: { type: 'fixed', value: 10 } });
      subscriptionRepo.findOne.mockResolvedValue({ plan: { benefits: { freeLeads: 50 } } } as any);
  ...
  describe('Customer Membership Plans', () => {
    const dto = { name: 'Gold', monthlyPrice: 199, yearlyPrice: 1999, benefits: { feeWaiverPercent: 100 } };
    ...
    it('creates a membership plan', async () => {
      ...
      const result = await service.createMembershipPlan(dto as any);
      expect(result).toEqual(dto);
    });
    ...
    it('updates a membership plan', async () => {
      membershipPlanRepo.findOne.mockResolvedValue({ id: 'mp-1', name: 'Silver' });
      membershipPlanRepo.save.mockImplementation((e: any) => Promise.resolve(e));
      const result = await service.updateMembershipPlan('mp-1', { name: 'Platinum' });
    ...
    describe('assignCustomerMembership', () => {
      it('assigns monthly membership', async () => {
        ...
        membershipPlanRepo.findOne.mockResolvedValue({ id: 'plan-1', name: 'Gold' });
        customerMembershipRepo.update.mockResolvedValue(undefined);
        customerMembershipRepo.create.mockReturnValue({ id: 'cm-1', billingCycle: 'monthly' } as any);
        customerMembershipRepo.save.mockResolvedValue({ id: 'cm-1', billingCycle: 'monthly' } as any);

        const result = await service.assignCustomerMembership('user-1', 'plan-1', 'monthly', 'admin-1');
      ...
      it('assigns yearly membership', async () => {
        ...
        membershipPlanRepo.findOne.mockResolvedValue({ id: 'plan-1', name: 'Gold' });
        customerMembershipRepo.update.mockResolvedValue(undefined);
        customerMembershipRepo.create.mockReturnValue({ id: 'cm-1', billingCycle: 'yearly' } as any);
        customerMembershipRepo.save.mockResolvedValue({ id: 'cm-1', billingCycle: 'yearly' } as any);

        const result = await service.assignCustomerMembership('user-1', 'plan-1', 'yearly', 'admin-1');
    ...
    describe('getCustomerFeeWaiver', () => {
      it('returns waiver when membership has feeWaiverPercent', async () => {
        customerMembershipRepo.findOne.mockResolvedValue({ plan: { benefits: { feeWaiverPercent: 100 } } } as any);
