...
export class TranslationService {
  ...
  detectLanguage(text: string): SupportedLanguage {
    ...
    return hindiPattern.test(text) ? SupportedLanguage.HI : SupportedLanguage.EN;
  }

  async translate(text: string, targetLang: SupportedLanguage): Promise<TranslationResult> {
    if (!text || text.trim().length === 0) {
      return { original: text, translated: text, sourceLang: 'unknown', targetLang };
    }
    ...
    const translated = lookupTable[text] || this.placeholderTranslate(text, targetLang);
  ...
  private placeholderTranslate(text: string, targetLang: SupportedLanguage): string {
