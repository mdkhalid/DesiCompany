...
describe('CashGateway', () => {
  ...
  describe('createOrder', () => {
    ...
    it('produces different gatewayOrderIds on successive calls', async () => {
      const a = await gateway.createOrder({
        amount: 100,
        currency: 'INR',
        bookingId: 'b1',
      });
      const b = await gateway.createOrder({
        amount: 200,
        currency: 'INR',
        bookingId: 'b2',
      });
  ...
  describe('refund', () => {
    ...
    it('produces different refundIds on successive calls', async () => {
      const a = await gateway.refund({ gatewayPaymentId: 'cash_123' });
      const b = await gateway.refund({ gatewayPaymentId: 'cash_456' });
