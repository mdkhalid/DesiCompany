...
describe('PaymentsService', () => {
  ...
  describe('createOrderForBooking', () => {
    ...
    it('throws NotFoundException when booking not found', async () => {
      ...
      await expect(service.createOrderForBooking(bookingId, userId, UserRole.CUSTOMER))
        .rejects.toThrow(NotFoundException);
    });

    it('throws ForbiddenException when wrong customer', async () => {
      bookingRepo.findOne.mockResolvedValue(mockBooking as any);
      await expect(service.createOrderForBooking(bookingId, 'wrong-user', UserRole.CUSTOMER))
        .rejects.toThrow('You can only pay for your own bookings');
    });

    it('throws BadRequestException when booking not completed', async () => {
      bookingRepo.findOne.mockResolvedValue({ ...mockBooking, status: BookingStatus.WORKING } as any);
      await expect(service.createOrderForBooking(bookingId, userId, UserRole.CUSTOMER))
    ...
    it('throws ConflictException when already paid', async () => {
      bookingRepo.findOne.mockResolvedValue(mockBooking as any);
      paymentRepo.findOne.mockResolvedValue({ id: 'pay-1', status: PaymentStatus.SUCCESS } as any);
      await expect(service.createOrderForBooking(bookingId, userId, UserRole.CUSTOMER))
    ...
    it('creates online payment order via gateway', async () => {
      ...
      factory.getDefault.mockResolvedValue(mockGateway as any);
      ...
      paymentRepo.create.mockReturnValue({ id: 'pay-new' } as any);
      paymentRepo.save.mockResolvedValue({ id: 'pay-new' } as any);
    ...
    it('creates cash order when default gateway is cash', async () => {
      ...
      bookingRepo.findOne.mockResolvedValue(mockBooking as any);
      paymentRepo.findOne.mockResolvedValue(null);
      factory.getDefault.mockResolvedValue(cashGateway as any);
      paymentRepo.create.mockReturnValue({ id: 'pay-cash' } as any);
  ...
  describe('getPaymentStatus', () => {
    ...
    it('throws NotFoundException when payment not found', async () => {
      paymentRepo.findOne.mockResolvedValue(null);
      await expect(service.getPaymentStatus(paymentId, userId, UserRole.CUSTOMER))
        .rejects.toThrow(NotFoundException);
    });
    ...
    it('returns cached status when payment is not pending', async () => {
      paymentRepo.findOne.mockResolvedValue({
        ...
        booking: { customer: { user: { id: userId } }, provider: { user: { id: 'p-user' } } },
      } as any);

      const result = await service.getPaymentStatus(paymentId, userId, UserRole.CUSTOMER);
    ...
    it('polls gateway for pending payments and updates on success', async () => {
      ...
      mockGateway.getStatus.mockResolvedValue({ status: 'success', gatewayPaymentId: 'pay_xyz', gatewayOrderId: 'order_abc', amount: 50000 });
  ...
  describe('payCash', () => {
    it('creates cash payment record', async () => {
      ...
      const result = await service.payCash('booking-1', userId, UserRole.CUSTOMER);
  ...
  describe('markCashReceived', () => {
    it('marks cash payment as success and credits wallet', async () => {
      ...
      walletRepo.create.mockReturnValue({ balance: 0, user: { id: providerUserId } } as any);
      walletRepo.save.mockResolvedValue({} as any);
      txRepo.create.mockReturnValue({} as any);

      const result = await service.markCashReceived('booking-1', providerUserId, UserRole.PROVIDER);
