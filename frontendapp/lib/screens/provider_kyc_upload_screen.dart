...
class _ProviderKycUploadScreenState extends State<ProviderKycUploadScreen> {
  ...
  Future<void> _pickFromGallery() async {
    try {
      ...
      if (photos.isNotEmpty) setState(() => _selectedPhotos.addAll(photos));
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.toString())));
    }
  }
  ...
  Future<void> _upload() async {
    ...
    try {
      ...
      final request = http.MultipartRequest('POST', Uri.parse('${ApiService.baseUrl}/kyc/upload'));
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        setState(() { _selectedPhotos.clear(); _uploading = false; });
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(loc.tr('uploaded_success')), backgroundColor: AppTheme.secondary));
        await _loadExisting();
      } else {
        setState(() => _uploading = false);
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(body.isNotEmpty ? body : 'Upload failed'), backgroundColor: AppTheme.error));
      }
    } catch (e) {
      if (mounted) {
        setState(() => _uploading = false);
          ...
          : _error != null
              ? Center(child: Padding(
                  ...
                  child: Column(mainAxisSize: MainAxisSize.min, children: [
                    ...
                    const SizedBox(height: 16),
                    ElevatedButton(onPressed: _initialize, child: Text(loc.tr('retry'))),
                  ]),
                ))
              : RefreshIndicator(
  ...
  Widget _buildUploadSection(LocalizationProvider loc) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Text(loc.tr('add_photos'), style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: AppTheme.textPrimary)),
        ]),
        const SizedBox(height: 16),
        Container(
          ...
          child: DropdownButtonHideUnderline(
            child: DropdownButton<String>(
              ...
              items: _docTypes.entries.map((e) => DropdownMenuItem(value: e.value, child: Text(e.key, style: const TextStyle(fontSize: 14, color: AppTheme.textPrimary)))).toList(),
              onChanged: (v) { if (v != null) setState(() => _selectedDocType = v); },
            ),
          ),
        ),
        const SizedBox(height: 16),
        Row(children: [
          Expanded(child: OutlinedButton.icon(
            ...
            style: OutlinedButton.styleFrom(foregroundColor: AppTheme.primary, side: const BorderSide(color: AppTheme.primary), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), padding: const EdgeInsets.symmetric(vertical: 14)),
          )),
          const SizedBox(width: 12),
          Expanded(child: OutlinedButton.icon(
        ...
        if (_selectedPhotos.isNotEmpty) ...[
          ...
          SizedBox(
            ...
            child: ListView.separated(
              ...
              itemBuilder: (_, i) => _buildLocalThumbnail(i),
            ),
          ),
          const SizedBox(height: 16),
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: Text(_uploading ? loc.tr('uploading') : loc.tr('upload')),
  ...
  Widget _buildLocalThumbnail(int index) {
    final xfile = _selectedPhotos[index];
    return Stack(clipBehavior: Clip.none, children: [
      ClipRRect(
        borderRadius: BorderRadius.circular(10),
        child: Container(width: 80, height: 80, color: Colors.grey.shade100,
          child: Image.network(xfile.path, width: 80, height: 80, fit: BoxFit.cover,
            errorBuilder: (_, __, ___) => const Icon(Icons.image, size: 32, color: AppTheme.textSecondary)),
        ),
      ),
      Positioned(top: -6, right: -6,
        child: GestureDetector(
          onTap: () => _removePhoto(index),
          child: Container(width: 22, height: 22, decoration: const BoxDecoration(color: AppTheme.error, shape: BoxShape.circle),
      ...
      child: Container(width: 80, height: 80, color: Colors.grey.shade100,
        child: Image.network(url, width: 80, height: 80, fit: BoxFit.cover,
          errorBuilder: (_, __, ___) => Container(
            ...
            child: const Center(child: Icon(Icons.broken_image, size: 24, color: Colors.white70)),
          ),
        ),
      ),
    );
  ...
  Widget _buildExistingSection(LocalizationProvider loc) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(loc.tr('kyc_documents'), style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: AppTheme.textPrimary)),
      const SizedBox(height: 12),
      if (_existingDocs.isEmpty)
        Container(padding: const EdgeInsets.symmetric(vertical: 40), alignment: Alignment.center,
          child: Column(children: [
            Icon(Icons.folder_open, size: 56, color: Colors.grey.shade300),
            const SizedBox(height: 12),
            Text(loc.tr('no_documents_yet'), style: const TextStyle(color: AppTheme.textSecondary, fontSize: 14)),
          ]),
        )
      else
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Row(children: [
              ...
              Text(_docTypeLabel(docType), style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: AppTheme.textPrimary)),
            ]),
          ...
          if (files.isNotEmpty) ...[
            const SizedBox(height: 12),
            SizedBox(height: 80,
              child: ListView.separated(scrollDirection: Axis.horizontal, itemCount: files.length, separatorBuilder: (_, __) => const SizedBox(width: 8),
                itemBuilder: (_, i) {
                  final f = files[i];
                  final url = f is Map ? (f['path'] ?? f['url'] ?? '') : f.toString();
                  return _buildNetworkThumbnail(url);
                },
              ),
            ),
          ],
          if (status == 'rejected' && remarks != null && remarks.isNotEmpty) ...[
            ...
            Container(padding: const EdgeInsets.all(10),
              decoration: BoxDecoration(color: AppTheme.error.withValues(alpha: 0.05), borderRadius: BorderRadius.circular(10), border: Border.all(color: AppTheme.error.withValues(alpha: 0.2))),
              child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Icon(Icons.comment, size: 14, color: AppTheme.error),
                const SizedBox(width: 8),
                Expanded(child: Text(remarks, style: const TextStyle(fontSize: 12, color: AppTheme.error))),
              ]),
            ),
          ],
        ]),
